...
redirect_result(){
    ...
    then  
        ima "$1" < src/test/deca/extension/codegen/valid/Input_read_operation.txt > src/test/deca/extension/codegen/valid/result/$(basename "${1%.ass}")_output.res 
    else 
        ima "$1" > src/test/deca/extension/codegen/valid/result/$(basename "${1%.ass}")_output.res 
        compare_output src/test/deca/extension/codegen/valid/result/$(basename "${1%.ass}")_output.res
    fi
}
...
for i in src/test/deca/extension/codegen/valid/*.deca
do
    #echo "$i"
...
for i in src/test/deca/extension/codegen/valid/*.ass
do
    # echo "$i"
    redirect_result "$i"
done
