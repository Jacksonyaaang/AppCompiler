...
SCR='\U1F631'
GREENFOND="\e[42m"
echo -e "${GREENFOND} AUTO VALID SYNTAX TEST START : ${ENDCOLOR} \n" 
...
redirect_result_decompile(){
    mkdir -p src/test/deca/extension/syntax/valid/result
    decac -p "$1" > src/test/deca/extension/syntax/valid/result/$(basename "${1%.deca}")_decompile.lis 2>&1 &
}

redirect_result(){
    mkdir -p src/test/deca/extension/syntax/valid/result
    test_synt "$1" > src/test/deca/extension/syntax/valid/result/$(basename "${1%.deca}").lis 2>&1 &
}


for i in src/test/deca/extension/syntax/valid/*.deca
do
