...
redirect_result(){
    ima "$1" > src/test/deca/codegen/invalid/result/$(basename "${1%.ass}")_output.res 
    compare_output src/test/deca/codegen/invalid/result/$(basename "${1%.ass}")_output.res
}
...
redirect_result_no_check (){
    ima "$1" > src/test/deca/codegen/invalid/result/$(basename "${1%.ass}")_output_no_check.res 
    compare_output_no_check src/test/deca/codegen/invalid/result/$(basename "${1%.ass}")_output_no_check.res
}
...
for i in src/test/deca/codegen/invalid/*.ass
do
    # echo "$i"
    redirect_result "$i"
done
