...
redirect_result(){
        ima "$1" < src/test/deca/codegen/interactive/$(basename "${1%.ass}")_input.txt > src/test/deca/codegen/interactive/result/$(basename "${1%.ass}")_output.res 
        compare_output src/test/deca/codegen/interactive/result/$(basename "${1%.ass}")_output.res
...
for i in src/test/deca/codegen/interactive/*.deca
do
    # echo "$i"
...
for i in src/test/deca/codegen/interactive/*.ass
do
    # echo "$i"
    redirect_result "$i"
done
